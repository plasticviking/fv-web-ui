...
public class OperationsTests {
  ...
  @Mock
  @RuntimeService
  CleanupCharactersService cleanupCharactersService;
  ...
  public void testOperationsInitPhase() throws RuntimeException {
    operations.forEach((k, v) -> {
      try {
        ...
        DocumentModel newDoc = session.getDocument(dialectDoc.getRef());

        List<String> requiredJobs = PropertyUtils
            .getValuesAsList(newDoc, CommonConstants.REQUIRED_JOBS_FULL_FIELD);
  ...
  @Test
  public void testCleanConfusablesIgnoredIfAddConfusablesPresent()
      throws OperationException {

    // Set AddConfusable operation to job
    dialectDoc.setPropertyValue(CommonConstants.REQUIRED_JOBS_FULL_FIELD,
  ...
  @Test
  public void testConfusableStatus()
    ...
    StringBlob response = (StringBlob) automationService
        .run(ctx, Constants.CONFUSABLES_STATUS_ACTION_ID);
