  ...
  private ObjectNode constructBookJSON(DocumentModel doc) {
    ...
    if (documentType.equalsIgnoreCase(FV_BOOK) || documentType.equalsIgnoreCase(FV_BOOK_ENTRY)) {
      ...
      if (documentType.equalsIgnoreCase(FV_BOOK)) {
        ...
        String[] authorIds = (!doc.isProxy())
            ? (String[]) doc.getProperty("fvbook", "author")
            : (String[]) doc.getProperty("fvproxy", "proxied_author");
        if (authorIds != null) {
          ArrayNode authorArray = mapper.createArrayNode();
          for (String authorId : authorIds) {
            ObjectNode authorObj =
                EnricherUtils.getDocumentIdAndTitleAndPathJsonObject(authorId, session);
            if (authorObj != null) {
