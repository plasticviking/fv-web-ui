...
public class PhraseEnricher extends AbstractJsonEnricher<DocumentModel> {
  ...
  private ObjectNode constructPhraseJSON(DocumentModel doc) {
    ...
    if (documentType.equalsIgnoreCase(FV_PHRASE)) {
      ...
      String[] phraseBookIds = (!doc.isProxy())
          ? (String[]) doc.getProperty("fv-phrase", "phrase_books")
      ...
      for (String phraseBookId : phraseBookIds) {
        ObjectNode phraseBookObj =
            EnricherUtils.getDocumentIdAndTitleAndPathJsonObject(phraseBookId, session);
