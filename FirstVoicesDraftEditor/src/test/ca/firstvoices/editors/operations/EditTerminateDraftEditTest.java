...
public class EditTerminateDraftEditTest {
  ...
  @Test
  public void editTerminateDraftEditTest() {
    ...
    testUtil
        .terminateDraftEditViaAutomation(automationService, draftEditorServiceInstance, docArray);

    session.save();

    docArray = testUtil.getTestWordsArray(session);

    assertTrue("Should have only live documents after draft session was terminated",
        docArray.length == initialWordCount);

    for (DocumentModel aWord : docArray) {
      String draftUuid = draftEditorServiceInstance.getUUID(aWord, DRAFT_UUID_REF);
      assertNull("Draft uuid should be removed", draftUuid);
