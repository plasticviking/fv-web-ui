...
@Operation(id = DocumentEnrichedQuery.ID, category = Constants.CAT_FETCH, label = "Enriched "
    + "Query", description = "Returns a query that is transformed, for example - includes a "
    + "lookup for sub-categories in addition to parent category")
public class DocumentEnrichedQuery {
  ...
  @Param(name = "enrichment", required = false, description = "Enrichment to perform on query",
      widget = Constants.W_OPTION, values = {
  ...
  @Param(name = "language", required = false, description = "The query language.", widget =
  ...
  @Param(name = PageProviderService.NAMED_PARAMETERS, required = false, description = "Named "
      + "parameters to pass to the page provider to fill in query variables.")
  ...
  public DocumentModelList run() throws OperationException {
    ...
    ElasticSearchNxqlPageProvider pp = (ElasticSearchNxqlPageProvider) PageProviderHelper
        .getPageProvider(session, def, namedParameters, sortBy, sortOrder, targetPageSize,
            targetPage, strParameters != null ? strParameters.toArray(new String[0]) : null);
