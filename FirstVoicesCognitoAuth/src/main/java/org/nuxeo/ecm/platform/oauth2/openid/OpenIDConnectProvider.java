...
/**
 * Class that holds info about an OpenID provider, this includes an OAuth Provider as well as urls
 * and icons
 *
 * @author <a href="mailto:nelson.silva@inevo.pt">Nelson Silva</a>
...
public class OpenIDConnectProvider implements LoginProviderLinkComputer {
    ...
    try {
      if (userResolverClass == null) {
        ...
        Constructor<? extends UserResolver> c =
            userResolverClass.getConstructor(OpenIDConnectProvider.class);
  ...
  public OpenIDUserInfo getUserInfo(String accessToken) {
    ...
    HttpRequestFactory requestFactory =
        HTTP_TRANSPORT.createRequestFactory(request -> request.setParser(new JsonObjectParser(
            JSON_FACTORY)));
    ...
    try {
      ...
      final String BearerMethod = OpenIDConnectProviderDescriptor.BEARER_AUTHENTICATION_METHOD;
      if (BearerMethod.equals(authenticationMethod)) {
  ...
  public OpenIDUserInfo parseUserInfo(String userInfoJSON) throws IOException {
    return new JsonObjectParser(JSON_FACTORY).parseAndClose(new StringReader(userInfoJSON),
        openIdUserInfoClass);
  }
