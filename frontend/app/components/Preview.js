...
import AudioOptimal from 'components/AudioOptimal'
...
import PreviewMetaDataContributor from 'components/PreviewMetaDataContributor'
    ...
    switch (this.props.type) {
      ...
      case 'FVCategory': {
        ...
        if (categoryResponse && category.success) {
          let breadcrumb = []
          // TODO: check to see if else branch can be removed
          if (this.props.expandedValue) {
            breadcrumb = (
              <>
                &raquo;
                {categoryResponse.title}
                {categoryResponse.path.indexOf('SharedData') !== -1
                  ? ` (${this.props.intl.trans('shared', 'Shared', 'first')})`
                  : ''}
              </>
            )
          } else {
            breadcrumb = (selectn('contextParameters.breadcrumb.entries', categoryResponse) || []).map((entry, i) => {
